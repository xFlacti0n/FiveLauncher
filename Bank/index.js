...
mp.events.add("client:bank:subMenu",(id,nummer) => {
    ...
    ui_sub.ItemSelect.on((item, index, value) => {
        if (item.Text == "Kontostand") {
          ...
          ui_sub.Close();          
            ...
            mp.events.callRemote("server:bank:abheben", item.Description);  
            ui_sub.Close(); 
...
mp.events.add("client:bank:openMenu",(json) => {
  ...
  const ui_main = new Menu("Bankkonten", "Wähle dein Bankkonto", MenuPoint);
    ...
    ui_main.ItemSelect.on((item, index, value) => {
      if (item.Text !== 'Schließen' && item.Text !== "Ausrauben") {
         mp.events.call("client:bank:subMenu",item.Description,item.Text)
         ui_main.Close();
        ...
        mp.events.callRemote("server:bank:ausrauben",playerposx,playerposy,playerposz);        
      } else {
        ui_main.Close();
      }         
    });
