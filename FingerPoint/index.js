...
let pointing = {
    ...
    stop: function() {
        ...
        mp.game.graphics.notify('Fingerpoint Stop clientside');
        mp.events.callRemote("pointingStop", mp.players.local);
    },
...
mp.keys.bind(0x42, true, () => {
    if (!mp.gui.cursor.visible && !mp.players.local.isBeingStunned(0)) {
        pointing.start();
    }
});

mp.keys.bind(0x42, false, () => {
    if (mp.players.local.getVariable("isUnconcious") == "false" && !mp.players.local.isBeingStunned(0)) {
        mp.events.callRemote("pointingStop", mp.players.local);
        pointing.stop();
    }
});
