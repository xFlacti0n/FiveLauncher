...
// MENÜ Interaktionen
mp.events.add("client:tequi:openInteractionMenu",(canInvite,ranks) => {
    const ui_main_interaction = new Menu("Tequi Interaktion", "Interagieren.", MenuPoint);
    ...
    ui_main_interaction.ItemSelect.on((item, index, value) => {
        if (item.Text == 'Einstellen') {
            ...
            mp.events.callRemote("server:tequi:hirePlayer", item.SelectedItem.DisplayText);
        } else if (item.Text == 'Entlassen') {
            mp.game.graphics.notify("Du entlässt die Person!");
            mp.events.callRemote("server:tequi:firePlayer");
...
// MENÜ FAHRZEUGE
mp.events.add("client:tequi:openDutyMenu", () => {
const ui_main_duty = new Menu("Tequilala Bar", "Tequilala Bar", MenuPoint);
    ...
    ui_main_duty.ItemSelect.on((item, index, value) => {
        if (item.Text == 'Dienst beginnen') {
            mp.events.callRemote("server:tequi:onDuty");
            ui_main_duty.Close();
        } else if (item.Text == 'Dienst beenden') {
            mp.events.callRemote("server:tequi:offDuty");
            ui_main_duty.Close();
        } else if (item.Text == 'Schließen') {
            ui_main_duty.Close();
        }
